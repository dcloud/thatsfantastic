...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--url",
            help="A URL to a Fantastic Fest movie list section.\
                                           Default: %(default)s",
        ...
        parser.add_argument(
            ...
            type=float,
            default=3.0,
            help="Number of seconds to wait for a request to complete"
        ...
        parser.add_argument(
            ...
            type=int,
            default=18,
            help="Number of films to per page. Used to calculate offset for"
            + " next page. Default: %(default)s",
        )
        parser.add_argument(
            "--start-page",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        page_page_sizes = range(
            self.start_page * self.page_size,
            self.page_size * self.max_pages,
            self.page_size,
        )
        page_urls = [
            urljoin(self.url, "P{0:d}".format(p_off)) for p_off in page_page_sizes
        ]
    ...
    def _collect_links_for_page(self, url):
        ...
        if response.ok:
            ...
            self.stderr.write(
                "Request for {} failed. Reason: {}".format(url, response.reason)
            )
