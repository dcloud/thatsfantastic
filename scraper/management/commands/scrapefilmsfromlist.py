...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=float,
            default=3.0,
            help="Number of seconds to wait for a request "
            + "to complete before giving up. Default: %(default)s",
        )
        parser.add_argument(
            "--page-size",
            type=int,
            default=18,
            help="Number of films to per page. Used to calculate "
            + "offset for next page. Default: %(default)s",
        )
        parser.add_argument(
            "--start-page",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        for url in film_urls:
            management.call_command(
                "scrapefilm", url, verbosity=1, savepath=self.savepath
            )
